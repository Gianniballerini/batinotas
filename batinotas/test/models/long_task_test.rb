...
class LongTaskTest < ActiveSupport::TestCase
  ...
  test "should create a task succesfully" do
      task = LongTask.new(description: 'a description', state: 'pending',percentage: 10, priority: 1, list: @list)
      assert task.save
  end

  test "should not save percentage out of range" do
    task= LongTask.create(description: 'a description', state: 'pending',percentage: 10, priority: 1, list: @list)
